...
fn sum_matrix_on_gpu_2d[
  nx:UInt,
  ny:UInt
](A:Float32Ptr, B:Float32Ptr, C:Float32Ptr):
  ix:UInt = thread_idx.x + block_idx.x * block_dim.x
  iy:UInt = thread_idx.y + block_idx.y * block_dim.y
...
fn main() raises:
    ...
    with DeviceContext(dev) as ctx:
      ...
      var grid = Dim(
        Int((nx + block.x() - 1) / block.x()), 
        Int((ny + block.y() - 1) / block.y())
      )

      print('Block:', block, "grid:", grid)

      with Timer("sum_matrix_on_gpu_2d <<< ({}, {}), ({}, {}) >>>".format(
        grid.x(),grid.y(), block.x(), block.y()
      )):
        ctx.enqueue_function[sum_matrix_on_gpu_2d[UInt(nx),UInt(ny)]](
