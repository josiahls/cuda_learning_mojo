...
__global__ void sumMatrixOnGPU2D(float *MatA, float *MatB, float *MatC, int nx,
  ...
  unsigned int idx2 = block_section_offset + idx;
...
int main(int argc, char **argv) {
  ...
  const int dimx = 8;
  const int grid_dimx = 16;
  const int dimy = 16;

  dim3 block(dimx, dimy);
  dim3 grid((nx + grid_dimx - 1) / grid_dimx, (ny + block.y - 1) / block.y);

  iStart = cpuSecond();
  sumMatrixOnGPU2D <<< grid, block >>> (d_MatA, d_MatB, d_MatC, nx, ny);
  cudaDeviceSynchronize();
  iElaps = cpuSecond() - iStart;
  printf("sumMatrixOnGPU <<< (%d, %d), (%d, %d) >>> Time: %3f\n", grid.x,
  ...
  Grid: 32, 32
    sumHost: 0.88 seconds
    sumGPU: 0.014327 seconds

  Grid 32, 16
    sumHost: 0.855 seconds
    sumGPU: 0.0139 seconds

  Grid 16, 16
    sumHost: 0.865 seconds
    sumGPU: 0.0142 seconds
