...
__global__ void sumMatrixOnGPU1D(float *MatA, float *MatB, float *MatC, int nx,
  ...
  // If nx is 4, and ny is 4
  // len of MatA/B/C are 16.

  // if ix == 0
  //    We assign global mem offsets: 0, 4, 8, 12
  // if ix == 3
  //   We assign global mem offsets: 3, 7, 11, 15
  
  // Note: How is this efficient?? This appears to be all over the place.
  // 

  // Get the block id basically. y is always 1.
  unsigned int ix = threadIdx.x + blockIdx.x * blockDim.x;

  if (ix < nx) {
    // Though the nx and ny are still 16k elements each
    // Basically for block 1 for example, we iterate through all the ys 
    // allocated for that block.
    for (int iy = 0; iy < ny; iy++) {
      // 
      int idx = iy * nx + ix;
...
int main(int argc, char **argv) {
  ...
  iElaps = cpuSecond() - iStart;
  printf("sumMatrixOnGPU1D <<< (%d, %d), (%d, %d) >>> Time: %3f\n", grid.x,
  ...
  Block Size 128, 1
    sumMatrixOnHost Time: 0.854966
    sumMatrixOnGPU1D <<< (128, 1), (128, 1) >>> Time: 0.018513
